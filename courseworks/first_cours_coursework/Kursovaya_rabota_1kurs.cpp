...
void dfs(int v, int len, bool** graph, bool* usedVertex, int* answer,int n) {
    ...
    for (int i = 0; i < n; ++i) { // Проходимся по всем вершинам графа
        if (graph[i][v] && !usedVertex[i]) {   // Если вершины i и v смежные и вершина i не была посещена
    ...
    if (n >= 0) {
        ...
        if (v >= 0) {
            ...
            if (v < n) {
                ...
                for (int i = 0; i < n; i++) {
                    if (answer[i] != 0) {  // выводим номера вершин из которых существует путь длины len к вершине v
                        out << i << " ";
                    }
                }
                if (graph[v][v] == 1) {
