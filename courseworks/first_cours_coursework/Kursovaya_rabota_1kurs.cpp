...
{
    if (len == 0)
    {                // Если длина пути достигнута, добавляем вершину в ответ
    ...
    { // Проходимся по всем вершинам графа
        if (graph[i][v] && !usedVertex[i])
        {                                                  // Если вершины i и v смежные и вершина i не была посещена
            dfs(i, len - 1, graph, usedVertex, answer, n); // Рекурсивно вызываем dfs для вершины i с уменьшенной на 1 длиной пути
        }
    }
...
int main()
{
    ...
    int sym1, sym2, sym3, sym4, sym5, sym6; // переменные для считывания данных с файлов
    for (int i = 0; i < 2; i++)
    { // считываем первые два числа с файла
        orgraph >> sym1;
    ...
    if (n >= 0)
    {
        bool *usedVertex = new bool[n]; // создаем динамический массив, хранящий посещенные вершины
        int *answer = new int[n];       // создаем динамический массив, хранящий ответы
        bool **graph = new bool *[n];   // создаем динамический массив - матрицу смежности графа
        for (int i = 0; i < n; i++)
        {
            graph[i] = new bool[n];
        }
        ...
        if (v >= 0)
        {
            ...
            if (v < n)
            {
                ...
                dfs(v, len, graph, usedVertex, answer, n); // запускаем агортим обхода в глубину
                out << "Номера вершин из которых существует путь длины " << len << " к вершине " << v << ": ";
                for (int i = 0; i < n; i++)
                {
                    if (answer[i] != 0)
                    { // выводим номера вершин из которых существует путь длины len к вершине v
                        out << i << " ";
                    }
                }
                if (graph[v][v] == 1)
            ...
            {
                out << "В вашем графе не содержится вершина с номером " << v << " (нумерация вершин начинается с 0)" << endl;
                return 0;
            }
        }
        else
